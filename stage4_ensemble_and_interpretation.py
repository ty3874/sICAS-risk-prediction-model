"""Stage 4: calibration, decision curve analysis, and SHAP interpretation."""
...
def main() -> None:
    ...
    y_prob_calibrated = cross_val_predict(
        calibrated_voting,
        X,
        y,
        cv=cv_eval,
    ...
    pd.DataFrame({"True_Label": y, "OOF_Prob_ML": y_prob_calibrated}).to_csv(
        output_dir / "OOF_Probabilities_For_Stage5.csv",
        index=False,
    )
    ...
    ax1.plot(
        ...
        color="#E63946",
        linewidth=2.5,
        label=f"Final Ensemble\n(Brier = {brier_cal:.3f})",
    )
    ax1.plot([0, 1], [0, 1], "k:", label="Perfectly calibrated")
    ax1.set(
        ylabel="Fraction of Positives (Observed)",
        xlabel="Mean Predicted Probability",
        title="A. Calibration Curve (Platt Scaling)",
    )
    ...
    ax2.plot(
        thresholds,
        calculate_net_benefit(y, y_prob_calibrated, thresholds),
        color="#E63946",
        linewidth=3,
    ...
    ax2.plot(
        thresholds,
        calculate_net_benefit(y, np.ones_like(y), thresholds),
        color="gray",
        linestyle="--",
        linewidth=2,
        label="Treat All",
    ...
    ax2.set(
        ...
        ylabel="Net Benefit",
        title="B. Decision Curve Analysis (DCA)",
    )
