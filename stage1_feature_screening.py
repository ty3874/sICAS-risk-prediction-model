...
def main() -> None:
    ...
    if not available_features:
        raise ValueError("No predefined features were found.")
    
    X_raw = df[available_features].copy()
    y = df[args.target].values

    categorical = [c for c in CATEGORICAL_COLUMNS if c in X_raw.columns]
    numerical = [c for c in X_raw.columns if c not in categorical]

    X_imputed = X_raw.copy()
    if categorical:
        X_imputed[categorical] = SimpleImputer(strategy="most_frequent").fit_transform(X_raw[categorical])
    if numerical:
        X_imputed[numerical] = IterativeImputer(max_iter=10, random_state=RANDOM_STATE).fit_transform(X_raw[numerical])
    ...
    lasso = LogisticRegressionCV(
        cv=cv_lasso, penalty="l1", solver="liblinear", scoring="roc_auc",
    ...
    rf = RandomForestClassifier(n_estimators=500, class_weight="balanced", n_jobs=-1, random_state=RANDOM_STATE)
    ...
    final_features_in_data = [f for f in FINAL_FEATURES if f in rf_importances.index]
    final_importance = rf_importances[final_features_in_data].sort_values()

    color_tier_a, color_tier_b, color_tier_c = "#E63946", "#F4A261", "#457B9D"

    if HAS_VENN:
        fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(14, 6))
        venn = venn2(
            subsets=(len(set_lasso - set_rfe), len(set_rfe - set_lasso), len(set_lasso & set_rfe)),
            set_labels=("LASSO", "RF-RFE"), ax=ax1
        )
    ...
    colors = [color_tier_a if f in tier_a else color_tier_b if f in tier_b else color_tier_c if f in tier_c else "#999999" for f in final_importance.index]
